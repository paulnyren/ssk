...
$query = "INSERT INTO singles_matches (winner, loser, match_date, winner_score, loser_score, court, referee, notes) VALUES(?,?,?,?,?,?,?,";
$executeArray = array();
...
array_push($executeArray, $params["winnerId"]);
$query = "UPDATE players SET elo_rank= ? WHERE player_id = ?;";
$statement = $pdo->prepare($query);
$statement->execute($executeArray);
$result = $statement->fetchAll();
print_r($query);
...
array_push($executeArray, $params["loserNewElo"]);
array_push($executeArray, $params["date"]);
$query = "INSERT INTO ELO_rank (player_id, singles_elo, date) VALUES (?, ?, ?);";
$statement = $pdo->prepare($query);
$statement->execute($executeArray);
$result = $statement->fetchAll();
print_r(json_encode($result));
...
$executeArray = array();
$query = "SELECT rank FROM singles_ladder WHERE player_id = ?";
array_push($executeArray, $params["winnerId"]);
$statement = $pdo->prepare($query);
$statement->execute($executeArray);
...
print_r(gettype($winnerRank));
print_r($loserRank);

if($winnerRank > $loserRank){
	$query = "UPDATE singles_ladder SET rank = rank + 1 WHERE rank < ? AND rank > ?";
	$executeArray = array();
	...
	$executeArray = array();
	$query = "UPDATE singles_ladder SET rank = ? WHERE player_id = ?";
	array_push($executeArray, $loserRank);
	array_push($executeArray, $params["winnerId"]);
	$statement = $pdo->prepare($query);
	$statement->execute($executeArray);	
